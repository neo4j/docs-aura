...
For example, insert the `$employee` parameter value into the URL e.g. `www.neo4j.com/name={employee}`
...
[WARNING]
====
Keep data secure! Avoid including sensitive data in URLs, because it will be exposed in browser history and server logs.
For example, the URL could include someone’s name, such as `?name=David` which means personal data is exposed directly in the URL.
====
