...
See xref:/dashboards/managing-dashboards.adoc#_exporting_the_dashboard_json_as_a_file[Exporting JSON as a file] and xref:/dashboards/managing-dashboards.adoc#_copying_the_dashboard_json_to_clipboard[Copying to clipboard] for export options.
...
* Select a NeoDash dashboard JSON that is stored in your database.
...
[NOTE]
====
Note that you only have the option to select a stored NeoDash dashboard JSON when there are stored dashboards in your database.

It is not possible to delete stored NeoDash dashboards from the database through the dashboard application.
====
