...
+
If no `Point` property is found, the visualization looks for properties named exactly `latitude` and `longitude`. Both `latitude` and `longitude` must exist.

. Fallback properties: `lat` / `long` (or `lon`)
+
If the `latitude` / `longitude` are not found, properties named exactly `lat` and `long` are used. Both `lat` and `long` must exist.
...
[.shadow]
image::dashboards/visualizations/servescity.png[width=700]
