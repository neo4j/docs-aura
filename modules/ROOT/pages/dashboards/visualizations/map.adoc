...
The visualization uses map tiles from link:https://www.openstreetmap.org/about[OpenStreetMap] but they are served through Neo4j’s own tile service rather than public OpenStreetMap APIs, so no data is sent to external services.
...
First, create an Aura instance, then go to "Query".
Next, in the Cypher editor, copy and paste the following Cypher and execute the query:
...
.Example: Zoom in of airport serving a city nodes
[.shadow]
image::dashboards/visualizations/servescity.png[]

After editing the styling to add name captions, it's now clear that Arlanda Airport serves Stockholm.

.Example: Arlanda airport
[.shadow]
image::dashboards/visualizations/arlandaairport.png[]
