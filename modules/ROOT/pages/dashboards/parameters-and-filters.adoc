...
=== Step 6: Use the filter in a card

The parameter and filter are now connected.

Selecting a value from the dropdown updates the parameter `$unit_param` and dynamically updates the table.
...
Paste the following query into the Cypher input:
...
In your dashboard, the filter and the resulting table appear together. Changing the city in the filter updates the table dynamically.
