...
You have to create a filter and you have to link it to a card manually with a parameter and this is the main way that you can make cards kind of no longer static but make them interactive.

People want to do different kinds of filterings. 
They might want to filter by a date. 
They might want to select a a number value. 
They might want to select a free text value. 
So what what we see people do, for example, is they want to create a kind of mini search engine like a experimental search engine where they have a free text filter where they can enter any any words and then in the visualizations they will filter out based on on that set that was filtered. 
Even with AI, people could use a free text filter to write, let's say, a a prompt that they will then encode into an embedding and then use that embedding to do vector search. 
So, people will do all kinds of mini app development with with filters as well.
