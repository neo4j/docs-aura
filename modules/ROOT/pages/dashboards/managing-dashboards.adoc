= Managing dashboards
:description: Create and modify Neo4j dashboards in the Aura console. First, you'll need some data, and a data model, then once you have set up your knowledge graph you can let AI create charts with different data visualisations, or create dashboards from scratch starting with an empty dashboard. Or use both! Get started with AI and then refine it with Cypher and editing the styling.

In Neo4j Aura, use **Dashboards** under **Tools** in the left-side navigation to access the dashboard page.

[NOTE]
====
The number of dashboards you can create depends on your Aura tier:

* AuraDB Free: three dashboards
* AuraDB Professional: 25 dashboards
* AuraDB Business Critical and Virtual Dedicated Cloud: unlimited dashboards
====

== Editing a dashboard

. Select a dashboard tile from the **Dashboards** page.
. To change the title, select the title text and edit it, then confirm.
. (Optional) Add a description of a dashboard

== Dashboard pages

A manually created dashboard contains a single page "Main page".
Edit or duplicate the page via hovering the page tab and using the more menu *[...]*.
Add more pages via **+** next to the existing tabs.
Additional pages can also be deleted from the more menu *[...]*.

== Dashboard cards

A dashboard page can hold any number of cards.
Each card can hold a xref::/dashboards/visualizations/index.adoc[visualization], also referred to as the **Chart type**.
To add a card, use **Add card** at the bottom right of the screen.

* Use the more menu *[...]* at the top right of a card to edit the card.
* Use the six dots at the top left of a card to drag and drop it within the grid of the page.
* Hover over a card and use the arrow in the bottom right to resize it.

[.shadow]
.Drag and drop, edit or resize a card
image::dashboard-card-menu.png[width=700]

== Deleting a dashboard

To delete a dashboard, on the Dashboards page, select the more menu *[...]* on the dashboard tile, select *Delete*, and confirm.

Only the `Owner` of a dashboard can delete it. 
The *Delete* option is not shown to other roles.
In Card View, on the bottom left corner of the dashboard tile there is a blue circle with the `Owners` initials.

== Exporting the dashboard JSON as a file

In the dashboards tile page, use the more menu *[...]* of a dashboard tile and select **Export file...**.
Neo4j Aura creates a dashboard JSON file with the following format: `[dashboard title]_[current date as YYYY-MM-DD].json`.


== Copying the dashboard JSON to clipboard

In the dashboards tile page, use the more menu *[...]* of a dashboard tile and select **Copy to clipboard**.
