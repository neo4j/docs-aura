...
When you run graph algorithms in Bloom, these either run in a separate compute environment via xref:graph-analytics/aga.adoc[Aura Graph Analytics], or they run in your instance using the Graph Analytics plugin, sharing both memory and compute with your database.
This is determined by which option you chose when you created the instance, as described in xref:getting-started/create-instance.adoc[Create instance] and cannot be manually selected from Bloom.
