...
Additionally, running an algorithm on the current Scene only temporary adds a score to the elements visible in the Scene.
This action does *not* write to your graph.
But creating a projection on the entire graph adds a new database property to *all affected elements* containing the score for that element.
This is *written* to the graph and unlike other property keys, it cannot be altered.
You can manually remove the property using the *Query* tool, if needed.
