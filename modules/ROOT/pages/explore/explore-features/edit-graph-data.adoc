:description: This section describes how to edit graph data in Bloom.
...
* *Create new relationships* - New relationships can be created from the scene directly.
...
* *Create new nodes* - New nodes can also be created from the scene.
To create an empty new node, use the scene context menu and select an existing category to which the node should be assigned.
...
* *Delete a relationship* - A relationship can be deleted from the scene as well.
...
* *Delete nodes* - Similarly, nodes can also be deleted directly from the scene.
...
[WARNING]
====
Deleting nodes and relationships from the scene permanently deletes them from the database.
