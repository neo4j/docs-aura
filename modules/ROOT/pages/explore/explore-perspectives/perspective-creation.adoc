:description: This section describes how to create and use Perspectives in Bloom.
...
When you connect Bloom for the first time to a non-empty database, a Perspective is automatically created for you (see *Generate Perspective* below for details on how this is done.).
...
[NOTE]
====
The default Perspective and any Scenes belonging to the default Perspective are *not* shareable.
====
...
Bloom assigns a default color for the category, but you can change the default color and node sizes from the xref:explore/explore-visual-tour/legend-panel.adoc[Legend panel].
