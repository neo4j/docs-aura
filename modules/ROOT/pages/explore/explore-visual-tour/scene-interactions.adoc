...
The _hierarchical_-layout option from the layout menu located at the bottom right-hand corner of the scene, presents the nodes in the Scene in an hierarchical order instead.
...
The third option is the _coordinate layout_ and it arranges, and fixes, the nodes on the scene by their `integer`, `float`, or `point` type properties (provided that the nodes have them) and is used for geographical entities for example.
