...
* *User* (authorization code): Log in via the Aura console, using your credentials.
You may need to enable the newly added MCP server in the client before the client can invoke it.

* *Machine-to-machine* (client credentials): Service-to-service access using long-lived credentials issued by console-api.
...
. Create credentials
+
Call `POST /oath/genai` on console-api with a Console JWT.
Include `https://agent-mcp.neo4j.io` (or any other environment-specific audience) in the `Audience` array.
The response contains a `ClientID` and a `ClientSecret`, store them securely immediately as the latter is only returned this once.
+
