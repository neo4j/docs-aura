...
Note that you need to have link:https://nodejs.org/en[node.js] v >=24.13 installed to add a local MCP server.

. Locate the `claude_desktop_config` JSON file.
You can find this from the desktop via Settings/Developer:
+
image::claude-config.jpg[width=400]
+
Add a local MCP server:
+
[source,json]
...
+
image::claude-desktop.jpg[width=400]
+
