...
In the case of `429 Too Many Requests`, it is recommended to slow down the rate of all requests sent from your client application and consider retrying with a suitable minimum delay and backoff strategy.
...
[NOTE]
====
The endpoints related to billing, `/organizations/{org_id}/billing/ledger` and `/organizations/{org_id}/billing/usage`, available in *v2beta1*, have a rate limit of *10 requests per day and organization*.
====
