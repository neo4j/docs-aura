...
[NOTE]
====
Snapshots are kept in the system for 7 days for Professional instances, 30 days for Free instances, and 90 days footnote:[Scheduled snapshots are kept in the system for 14 days for Enterprise instances.] for Enterprise instances. Only the latest snapshot is available for Free instances.
====
...
* label:AuraDB-Professional[] label:AuraDB-Enterprise[] *Scheduled* - Runs when you first create an instance, when changes to the underlying system occur (for example, a new patch release), and automatically at a cadence depending on your plan type. Scheduled snapshots are run automatically once a day for Professional instances and once an hour for Enterprise instances.
