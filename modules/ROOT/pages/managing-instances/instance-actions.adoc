...
[NOTE]
====
Available for AuraDB pre-paid plans on Google Cloud Platform (GCP), version 5.x.
====

Aura instances have flexible storage, so you can increase storage independent of memory.
Storage can be selected when creating the instance, or later when your needs change.
