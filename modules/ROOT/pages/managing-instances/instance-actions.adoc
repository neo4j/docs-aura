...
When Neo4j updates Aura Database Versions, instances marked as `production` are last to have the Aura Database Version updated. Because updates are applied to these database instances after other database instances recieve updates, you can monitor any potential impact on less critical databases first.

After marking the instance as `production` the label is applied immediately, and all instance actions (such as pause or clone) will be temporarily unavailable while the instance is set to `production` on the Neo4j side. This should only take a few moments.

To mark an instance as `production` click the more actions (three dots) on the instance card, then "Mark as production".