...
=== Update to the upgrade flow

The *clone to New method* available via the Aura console and Aura API, previously created a new AuraDB Business Critical instance with a different DBID. 
Now, AuraDB Professional instances can be upgraded to Business Critical without downtime and without changing the database ID.
The upgrade scales the AuraDB Professional single-node cluster to an AuraDB Business Critical three-node Business Critical cluster and enables all associated features.
