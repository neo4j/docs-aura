...
[NOTE]
====
Available for AuraDB prepaid billing on Google Cloud Platform (GCP).
====

Adjust the storage size of your instance at any time.

To select the storage size during instance creation, select the value in the *Storage (Adjustable)* column. This opens a drop down with other available options.

To adjust an existing instance's storage, select *Configuration* on the instance card and choose a new value in the *Storage (Adjustable)* column. (See Figure 2.)

[.shadow]
.Adjust storage size
image::adjustablestorage.png[screenshot of instance storage resize]
