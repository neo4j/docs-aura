...
====
Available for:
* AuraDB pre-paid plans.
* Google Cloud Platform (GCP)
* version 5.x
====

Aura instances have flexible storage, so you can increase storage without increasing memory, which is ideal for running heavy queries or for growing graphs.

Why? Meet varying workloads and improve cost efficiency.

Adjust storage at any time by selecting *Configuration* on the instance card then *Instance Size* and select the value in the Storage (adjustable column) which is only available for GCP.

[.shadow]
image::instancesizeconfig.png[screenshot of instance size configuration]

Storage can be changed when creating the instance, or it can be done by resizing an instance.
