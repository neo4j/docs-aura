...
Instance actions (such as pause or clone) are temporarily unavailable while size or storage is changing.
...
You can’t downsize to a storage size smaller than what you're currently using. 
For example, if your storage usage is 8 GB, you won’t be able to downsize to 6 GB.
