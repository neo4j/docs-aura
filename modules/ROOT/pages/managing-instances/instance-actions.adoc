...
----
For more info see the link:https://neo4j.com/docs/aura/platform/api/specification/?urls.primaryName=Aura%20v1#/instances/post-resume-instance[Aura API spec]
=====
====
...
Returns the instance ID, name, status, connection URL, creation time, metrics integration URL, region, instance type, cloud provider, tenant ID, memory, storage, customer-managed key ID, graph nodes, graph relationships, secondaries count, CDC enrichment mode, vector optimization status, and Graph Analytics plugin.

For more info see the link:https://neo4j.com/docs/aura/platform/api/specification/?urls.primaryName=Aura%20v1#/instances/get-instance-id[Aura API spec]
----
=====
====
...
Upgrades an AuraDB Professional instance to an AuraDB Business Critical instance.

Upgrades are not available for Marketplace projects or trial instances.

Optionally, provide values for memory and storage in the request body to resize the instance along with the upgrade. 
If you don’t provide values, the instance keeps its configuration. 
If you provide memory you must also provide storage and vice versa.
