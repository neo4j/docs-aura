...
The resultant output will default all users passwords to `newpassword` and the user will be required to change their password on initial log-on. 
...
You can export the output as CSV using `Download CSV`. 
...
The next step will be to create roles. 
This script will generate the cypher for roles that need to be created if they do not exist already.
...
You can verify the created roles using the command:
...
The user will be able to access the resource if their roles have aGRANT(allowlist) and do not have aDENY(denylist) relevant to that resource.
...
//Sample output is as follows:
//image::user-migration-recreation/showprivileges.png[Show privileges]
...
Therefore in the below script we will exclude the PUBLIC role and roles with no users mapped.
