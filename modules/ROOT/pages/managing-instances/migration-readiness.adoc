...
* Ensure you are using the system database and we do that with the `:use system` command
* Resultant output of below cypher queries can be played back on the cloned instance.
...
image::user-migration-recreation/showusers.png[Show users]

You can export the output as CSV using `Export CSV`. 
Copy the scripts from csv and execute them on the cloned Aura Instance . 
...
image::user-migration-recreation/showroles.png[Show roles]
...
You can now execute these commands directly to cloned Aura instance. 
...
When we created the users in the first step, by default all users will get the `PUBLIC` role which provides access to the default database and allows the execution of all procedures and user-defined functions. 
Therefore in the below script we will be excluding the PUBLIC role and roles having no users mapped.
...
image::user-migration-recreation/showroleswithusers.png[Show roles with users]
...
Please make sure you export the output of all of the above cypher queries and run every query to avoid any mismatch in the roles and mappings. 
To get help in case of an error, please open a support ticket.
