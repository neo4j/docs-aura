...
For Professional instances running the latest version, scheduled snapshots are run automatically once a day, same for both AuraDS Professional and for AuraDS Enterprise instances, and for Business Critical and Virtual Dedicated Cloud instances, they run once an hour.
