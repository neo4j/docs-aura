...
Snapshot and backup actions are not supported for instances with multiple databases enabled.

=== Import data into a database with `push-to-cloud`

For multiple databases in an instance, import data individually into a database in an instance using the `neo4j admin database upload` command and specify which database the file is uploaded to.

It already prompts you to specify the database name in database upload today.