...
The data in your single-database Aura instance can be backed up, exported, and restored using snapshots.
For instances containing multiple databases, this is done on a per-database basis.
However, you can *restore a whole instance*, as described in <<multi-db-instance-restore>> below.

A *snapshot* is a copy of the data in an *instance* at a specific point in time.
This includes the `system` database, which contains your RBAC and credentials.

A *backup* is a copy of the data in a *database* at a specific point in time and a snapshot of an instance consists of the backups of both the user database as well as the `system` database.
As such, the snapshot concept does *not* apply to instances with *multiple user databases*.
...
There are different kinds of snapshots taken at different intervals with different life cycles depending on your tier.
...
Scheduled and on-demand snapshots move through the same statuses.
...
|Pending
|The snapshot request has been processed and queued, but hasn't started yet.

|In Progress
|The snapshot is in progress. You can't restore from it or clone an instance from it until it completes.

|Completed
|The snapshot finished successfully and is available to restore. If exportable is true, you can also clone a new instance from it.

|Failed
|A reliable restore is not available from this snapshot, either because it didn't finish or because it is unsafe to restore.

|Cancelled
|The snapshot stopped before it finished, so no data was captured. This status is system generated and could appear if the instance was paused while the snapshot was in progress.
|===
...
[NOTE]
====
Any backups exported from Aura may be unrecovered.
For more information on how to recover a backup, see link:{neo4j-docs-base-uri}/operations-manual/current/backup-restore/aggregate/#aggregate-unrecovered-full-backup[Operations Manual -> Aggregating an unrecovered full backup].
====

Note that the ability to *Export or Create* an instance from a scheduled *Virtual Dedicated Cloud* snapshot is *limited to 14 days for 4.x instances*.
...
For instances with *multiple user databases*, only backups available for export have a *[...]* menu.
Such databases *cannot* be used to create new instances.
...
For instances with *multiple user databases*, you can restore the individual databases in the same way, but not the entire instance.
See <<multi-db-instance-restore>> for information on how to restore the whole instance.

[NOTE]
====
It is not possible to restore multiple databases within an instance at the same time, unless you are restoring the whole instance.
====
...
[NOTE]
====
Restoring from a link:{neo4j-docs-base-uri}/operations-manual/current/backup-restore/planning/#splitting-backup-archive[split backup archive] (introduced in Neo4j 2026.09) is *not* supported in Aura.
====
...
[CAUTION]
====
The `neo4j-admin database upload` command does not support uploads of link:{neo4j-docs-base-uri}/operations-manual/current/backup-restore/planning/#splitting-backup-archive[split backup/dump archives] introduced in Neo4j 2026.09.
====
...
[[multi-db-instance-restore]]
== Multi-database instance restore

If your instance contains multiple databases, backups are managed at the database level rather than the instance level.
As mentioned in the introduction, the concept of a snapshot cannot be applied to a multi-database instance.
However, you can create a *restoration point* that triggers backups of all the databases in the instance.

When you want to restore the instance to a previous state, you select a date and a time that you want to restore to and the available individual backups that match the specified timestamp are listed.
You can select to *restore* or *clear* each individual user database, i.e. *not* the `system` database.


From *Inspect > Backups* > use the *Select database* dropdown to select a database.
All snapshot, restore, and export actions apply to the selected database.

[.shadow]
.Multi-database backups
image::multidb/multidb-backups.png[]
