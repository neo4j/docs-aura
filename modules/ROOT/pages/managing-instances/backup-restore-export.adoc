...
For single-database instances, backups are managed through snapshots of the instance.
For xref:managing-instances/multiple-databases.adoc[MultiDB] enabled instances, backups are managed individually for each database.
...
== MultiDB instances (take backups only)
...
[.shadow]
.Multiple database (MultiDB) backups
