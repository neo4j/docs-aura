...
These snapshots can be used to restore data to a different Neo4j instance.
	...
	•	Experiment with algorithms on existing applications.
	•	Billed through the instance to customers.
	•	Provides scalable storage when compute and storage are decoupled.

== Graph Data Science plugin 

label:AuraDB-Professional[]

Test algorithms using data within AuraDB via the Graph Data Science plugin, and experiment with existing applications.
...
There's scalable storage when compute and storage are decoupled. Graph Data Science supports single member clusters, as a result running algorithms cannot be transferred between clusters. Billed through the instance, the Graph Data Science requires approximately 300mb of memory when running or paused, therefore you can enable or disable the plugin when necessary.
