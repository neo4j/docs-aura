...
This feature allows you to better organize and manage multiple projects under a single organization.

* Organizations are useful for SSO, user management.
* There can be a portfolio of companies using one account. 
* The ability to rollout those Project level settings in one go, rather than having to configure them one at a time which is a  a time-saver that’s really useful because it lets you make changes at an overarching level, applying them across all projects at once.
* `Organization owners` are a new type of role.
