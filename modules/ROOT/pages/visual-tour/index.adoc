...
image::org-project-navigation.jpg[Organization/project navigation]
...
You can edit the name of the organization, find your organization ID, change your Organization default Cypher version, enable Generative AI and Aura Graph Analytics.

.Organization settings
[.shadow]
image::organization-settings.jpg[]

=== Generative AI assistance

When enabled, Generative AI assistance is available in a number of places, including the xref:query/visual-tour.adoc#copilot[Query copilot] and xref:explore/explore-visual-tour/search-bar.adoc#copilot[Bloom copilot].
These features are identified with the sparkle icon and your use is subject to the link:{neo4j-docs-base-uri}/reference/license/#_genai_outputs[GenAI outputs] disclaimer.

=== Aura Agent

When enabled, you can create and deploy AI agents from the console.
Aura Agent requires Generative AI assistance to be enabled, if it is disabled, Aura Agent will also be disabled.
Aura Agent makes use of third-party AI models for planning, reasoning, retrieval, and explainability purposes as outlined in link:{neo4j-docs-base-uri}/reference/license/agent-ai-model/[License and disclaimers -> How Aura Agent uses AI models].
See xref:aura-agent.adoc[Aura Agent] for more information about agents.
...
From this page you can enable or disable xref:../graph-analytics/aga.adoc[Aura Graph Analytics], as well as configure the following settings:
...
This page lists the xref:../graph-analytics/aga.adoc[Aura Graph Analytics] sessions associated with a project.

By default, the list displays active sessions.
You can view past sessions by switching on the **Include deleted sessions** toggle.

For each session, the following details are available:

* *Session Identity:* Creation time, session name, and session ID.
* *Resources:* The Aura instance containing the data projected in the session (if any) and the memory allocated to the session (configurable in the <<org-settings>> page).
* *Ownership:* The user who created the session.
* *State:* The current session status and the time remaining to its configured time limit (for running sessions).

You can also perform the following actions via this page:

* You can terminate an active session using the **Delete** button.
* You can download the entire list in CSV format using the **Export** button. This export is useful for offline processing, such as combining session information with billing data for detailed consumption analysis.
...
The tools in the studio allow you to interact with your data and therefore require an active connection to an instance.
Once connected to an instance, you can use both the Bloom and the Query tools.
...
If your instance doesn't contain any data, the Import tool allows you to import CSV files to your database.
This service lets you create your data model and map it to your files.
See the xref:import/introduction.adoc[What is Import?] for more information.

=== Bloom

Bloom helps you visualize and interact with datasets without using any code.
...
Bloom is designed to make working with graph data more intuitive and insightful by providing a visual and interactive way to analyze and manage your data.

See xref:explore/introduction.adoc[What is Bloom?] for more information.
...
See xref:billing/cost-explorer.adoc[Billing] for more information.
