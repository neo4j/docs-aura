...
Download and install the latest version of the Java driver from the https://search.maven.org/artifact/org.neo4j.driver/neo4j-java-driver[Maven Repository] along with its listed dependencies.
...
[NOTE]
====
Developing with Neo4j Aura requires the handling of transient errors and retry management. One way this can be managed is through the use of https://neo4j.com/docs/java-manual/current/session-api/#java-driver-simple-transaction-fn[Transaction Functions].
====
