...
* *Serverless* - default and recommended, allows graph algorithms to run in a separate compute environment.
* *Plugin* - allows graph algorithms to run in the same instance, sharing resources. INSTANCE-DATABASE-DEFINITION-REVIEW

Once the instance is created, you *cannot* change this.

You can optionally enable:

* *xref:managing-instances/instance-details.adoc[Vector optimization]* to enhance performance for vector-based operations.
* *xref:managing-instances/multiple-databases.adoc[Multiple databases]* to use multiple databases in a single instance. 
