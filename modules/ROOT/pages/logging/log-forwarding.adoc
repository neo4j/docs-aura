...
When using log forwarding with GCP, all logs are transmitted to the link:https://docs.cloud.google.com/logging/docs/reference/v2/rest?rep_location=global[Cloud Logging API].
The log data is sent directly to `logging.googleapis.com` which resolves to the logging service of the current data center. At this point, the logs become available to you via Log Router without ever leaving the data center.
...
For cross-region log forwarding within Azure, the link:https://azure.microsoft.com/en-us/explore/global-infrastructure/global-network[Azure global network] backplane is used to transmit your logs securely without crossing the open internet.