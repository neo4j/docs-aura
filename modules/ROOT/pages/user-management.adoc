...
| View and open instances (we can view instances, and we open databases) INSTANCE-DATABASE-DEFINITION-REVIEW
| {check-mark}
| {check-mark}
| {check-mark}
| {check-mark}
...
| Clone data to new and existing databases
|
|
| {check-mark}
| {check-mark}
...
The predefined roles are assigned the following privileges:
...
The role allows access to metrics for all databases in a project.
...
The `METRICS_READER` role can view and open databases in the console, however, login to the instance is required to interact with it, with access to Explore and Query defined by the instance's RBAC settings.
