...
The console roles map to the privileges on Graph Analytics sessions described below.
...
| Delete own sessions
| {check-mark} footnote:role-change[Only possible if the user has previously created the session while having higher privileges.]
| {check-mark}
| {check-mark}
| {check-mark}
| {check-mark}
