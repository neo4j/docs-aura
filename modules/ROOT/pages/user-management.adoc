...
As an _Admin_, to invite a new user, you invite at the project level and assign a project-level role.

. Within a project, go to *Users* and select *Invite user*.
. Enter the **Email** address of the person you want to invite and press *Add*.
. Select the user's **Role**.
. Select **Invite**.
* The invited user will receive an email with a link to accept the invitation.
* The new user will appear within the list of users on the project **User** page with the `Pending invite` Status until they accept the invite.
* On accepting the invite, the invited user automatically gets an `ORG_MEMBER` role in the organization the project is part of
* If needed, you can change the org-level role after the invite is accepted - only do this if the user needs a different role at the organization level.
...
Users within a project can be assigned one of the following roles and these are assigned on invitation:
