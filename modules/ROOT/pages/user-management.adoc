...
To invite users you need the following role:

Invite users (organization level): `Organization admin`, `Organization owner`

Invite users (project level):  `Organization admin`, `Organization owner`, `Project admin`, `Project owner`

=== Invite users (project-level roles)

* Go to *Users* from within a project, and select *Invite users*.
...
=== Invite users (organization-level roles)

* Go to *Users* from within an organization, and select *Invite users*.
* Add one or more email addresses separated by commas.
* Select an organization role: `Owner`, `Admin`, or `Member`.
* Project access is optional. Select a role, then tick *Access* to confirm it.
...
//You can see pending invites from the *Pending tab*

== Edit users and roles

From the *Users* page, as an _Admin_, you can delete users or edit their roles using the [...] more menu by the user's name.
