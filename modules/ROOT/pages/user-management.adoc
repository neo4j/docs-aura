...
View and manage roles from *Users* pages, accessible via the console sidebar.
...
[IMPORTANT]
====
Every xref:visual-tour/index.adoc#_organizations[organization] must have an `Organization Owner`.
====
...
| Project Admin access to all projects in the organization
| {check-mark}
|
|
|===
...
The `METRICS_READER` role can view and open instances in the console, however, login to the instance is required to interact with it, with access to Bloom and Query defined by the instance's RBAC settings.
...
You can create and manage custom roles using a set of Cypher administration commands.
This allows for more fine-grained access control.
For more information, see link:{neo4j-docs-base-uri}/operations-manual/current/authentication-authorization/manage-roles/[Operations Manual -> Manage roles].

== Invite users

=== Invite from organization settings

* Go to xref:visual-tour/index.adoc#org-settings[organization settings] -> *Users*, and select *Invite users*.
* Add one or more email addresses separated by commas.
* Select an organization role.
* Project access is optional. Select a project role, then tick *Access* to confirm it.
* Review the invite summary before sending.

=== Invite from project settings

* Go to xref:visual-tour/index.adoc#_project_settings[project settings] -> *Users*, and select *Invite users*.
...
From the *Users* page, delete users or edit their roles using the [...] more menu by the user's name.
