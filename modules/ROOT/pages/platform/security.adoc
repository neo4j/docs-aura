...
Please note that within Aura there is a limit to one key per product per region.
...
=== AWS key  provider's side, there may be a delay between disabling a key and when the key can no longer be used to encrypt and decrypt data.
====
