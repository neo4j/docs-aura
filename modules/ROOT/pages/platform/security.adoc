...
[CAUTION]
====
This feature has been released as a public beta for AuraDB Enterprise and AuraDS Enterprise. Managed keys are currently AWS compatible, with GCP and Azure coming soon.
====
...
On the cloud provider's side, there may be a delay between disabling a key and when the key can no longer be used to encrypt and decrypt data.
====
