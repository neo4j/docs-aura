...
Please note that the Aura Console runs in a separate VPC, separate from the rest of Aura.

== Network Access

Neo4j provides authorized users with the ability to configure certain aspects of the infrastructure that is used to run and manage Aura instances, and the networking the platform leverages to establish secure connections between the database (Aura) and application VPCs.
...
To enable this, customers will need to create a private endpoint in their own VPC. 

== Private endpoints

Private endpoints are network interfaces inside your own VPC, which can only accessed within your private network. 
The cloud provider connects them over their backbone network to Neo4j Aura. 
