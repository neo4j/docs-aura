...
.Cluster Raft Is Leader
[frame="topbot", stripes=odd, grid="cols", cols="<1,<4"]
|===
| Metric name
m| neo4j_cluster_raft_is_leader
| Description
| Is this server the leader? Track this for each Core cluster member. 
It will report 0 if it is not the leader and 1 if it is the leader. The sum of all of these should always be 1. 
