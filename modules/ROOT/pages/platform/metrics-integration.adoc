 ...
 ** endpoint `+/projects/{projectId}/metrics-integration+` (for project metrics)
...
* Aura CLI has a subcommand for `projects` command to fetch project metrics endpoint:
+
[source]
----
aura projects get-metrics-integration --tenant-id <YOUR_TENANT_ID>
...
# extract endpoint
aura projects get-metrics-integration --project-id <YOUR_TENANT_ID> | jq '.endpoint'
----
