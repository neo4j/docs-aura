...
An application performance monitoring system can be configured to fetch metrics of Neo4j Aura Virtual Dedicated Cloud instances.
...
[CAUTION]
====
The legacy term `enterprise` is still used within the codebase and API. However, in the Aura console and documentation, the enterprise tenant type is now known as Virtual Dedicated Cloud.
====
