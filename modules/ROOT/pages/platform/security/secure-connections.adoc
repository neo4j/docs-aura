...
You can set up a second private link connection to applications that are hosted in a second region i.e. `us-west-1` but still housed inside the same Aura project.
...
To connect to your instance via Browser or Bloom, you must use a dedicated VPN. This is because when you xref: [disable public access] to your instance, this applies to all connections, including those from your computer when using Browser or Bloom.
...
Disable Public Access, this can be achieved via Aura Console > Security > Network Access.

And after clicking the checkbox for Disable Public Traffic on page 4 of 4 of the Wizard there is no detail 
* if this change requires a restart of the database instance?  
* if this enablement is immediate ( i.e. upon closing of the wizard / dialogue )?
* or if a cluster role etc is required. (Btw the answer is certainly not immediate ( i.e. completion of the wizard/dialogue).

But this is such a small component of the wizard and to which there is no doc for such at neo4j.com/docs/aura.   
So requesting a full doc of such
