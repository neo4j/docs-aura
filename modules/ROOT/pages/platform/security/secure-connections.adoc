...
To configure network access, you need to be authorized to access the part of [your system's?] infrastructure that runs and handles these instances as well as the networking used to establish secure connections between the database and the application's VPC.
This includes the ability to connect over the cloud provider's private link and private endpoint.

* *Public access:* If your Aura instances are public, traffic to them is allowed to traverse the public internet and they are accessible with the correct username and password.

* *Private access:* For your instance to be completely private, disable public traffic, use the cloud provider's network, and create a private endpoint inside your VPC, which gives you a private connection to Aura.
...
To configure settings for network access to your instance go to *Aura Console* > *Security* > *Network Access*.

From there, you can either set up a new network access configuration, or edit current configuration settings.
There is a step-by-step configuration guide within the Aura Console including:

. Choose your Aura instance info
. Create endpoint
. Endpoint connection requests
. You can disable public traffic in this step at any time. 
To continue accessing Browser and Bloom, you can configure a VPN (Virtual Privare Network) in your VPC and connect to these services over the VPN. 
There’s more information available > https://support.neo4j.com/s/article/13174783967507-How-To-Test-Connectivity-Through-The-Private-Endpoint
