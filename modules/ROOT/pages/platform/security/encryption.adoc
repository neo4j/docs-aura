...
Customer Managed Keys are designed to stay within the cloud provider’s secure environment to maintain their integrity and security.
...
[WARNING]
====
The loss of a Customer Managed Key — whether through deletion, disabling, or expiration — makes all data encrypted with that key inaccessible. Neo4j is unable to manage database instances if the key is disabled, deleted, or if permissions are revoked.
====

=== Delete a CMK in Aura

When managing a Customer Managed Key within Aura, it's important to note that if this key is being used to encrypt one or more Aura database instances, it cannot be deleted from the Aura console.

To delete the CMK from Aura, you must first delete any Aura database instances that are encrypted with this key. Once these instances are deleted, you can then proceed to delete the key from Aura. Please note that this process only removes the key from Aura's management — it does not delete the actual key from the Cloud KMS.
