...
Enable *purge protection*, *Azure role-based access control*, and *Azure Disk Encryption for volume encryption*, and allow access from all networks.
. When the deployment is complete, go to the *resource* tab and copy the *directory ID*.
. In the Azure portal, add role assignment by selecting relevant members as *Key Vault Administrator*.

=== Create a key

. Create a key in the Azure portal. Copy the *Key Identifier*, you need it in the next step.
. Go to *security settings* in the *Aura Console* and create a Customer Managed Key.
. Follow the instructions in the Aura Console for the next sections:

=== Create a service principal 

In the Azure tenant where your key is located, create a service principal linked to the Neo4j CMK Application with the *key identifier* displayed in the Aura Console.
...
. Select *Key Vault Crypto Officer* then *Select member* and assign access to *User, group, or service principal*.
. Paste the *Neo4j CMK Application ID*, then press *Select* and *Review + Assign*.
