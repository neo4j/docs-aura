...
| **API**          | Different CMK                       | Cloning blocked. Error message shown: `Can not clone this instance using API because the source instance is encrypted with a different CMK. Please use the console if you want to proceed with this action.`
| **API**          | CMK to Neo4j Managed Key            | Cloning blocked. Error message shown: `Can not clone to a Neo4j Managed Key encrypted instance because the source instance is encrypted with a CMK.`
| **Console**      | Same CMK                            | Cloning allowed.
| **Console**      | Different CMK or Neo4j Managed Key  | Cloning allowed. 
|===
