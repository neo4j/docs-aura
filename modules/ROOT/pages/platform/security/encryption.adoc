...
. Create a *key ring* ensuring the region matches your Aura database instance region. 
...
. We recommend you select a key rotation period. You can use default settings for other options.
. Select *Create* and you will be brought back to the key ring, with your key listed. 
. Click *More* (three dots) and *Copy resource name*, you need it in the next step. For more information on Google Cloud keys, see link:https://cloud.google.com/kms/docs/getting-resource-ids
. Go to *security settings* in the Aura Console and add a *Customer Managed Key*.
. When you have added the key in the Aura Console, you will see three service accounts in the Aura Console in a modal.
. Go to the Google Cloud console, click into the key and go to *Permissions* then *Grant Access* 
. In *Add principals* and paste the service accounts.
. In *Assign roles* assign both "Cloud KMS CryptoKey Encrypter/Decrypter" and "Cloud KMS Viewer" roles.
