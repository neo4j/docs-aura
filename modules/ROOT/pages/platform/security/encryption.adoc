...
CAUTION: It's crucial to differentiate between Azure Key Vault and Azure Key Vault Managed HSM for your key management needs. 
This document focuses on using *Azure Key Vault*.
...
Add a key in the Key Vault.
When the key is created, record the *Key Identifier* because you need it later.
...
. Go to *Access control (IAM)* and *Add role assignment.*
. Select *Key Vault Crypto Officer* to highlight the row in grey, and then press *next*.
. Tap *Select member* and assign access to *User, group, or service principal*, and paste the Neo4j CMK Application ID and add it to *Selected members*, then *Review + Assign*.