...
In the Azure portal, add role assignment by selecting relevant members as *Key Vault Administrator*.

=== Creating a key

Add a key in the Key Vault.
When the key is created, record the *Key Identifier*, you need it later.
...
For example, using Azure CLI the command is:
...
See https://learn.microsoft.com/en-us/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-examples[Azure documentation^] for more information.
