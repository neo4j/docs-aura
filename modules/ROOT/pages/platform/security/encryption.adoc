...
CAUTION: There's a difference between *Key vault* and *Azure Key Vault managed HSMs*. Don't get them confused. We want want the first one! The Azure Key Vault!

link:https://learn.microsoft.com/en-us/azure/key-vault/general/quick-create-portal[Create a key vault using the Azure portal]
...
* Ensure the region matches your Aura database instance region. 
* Acknowledge that you understand that you will be billed for the entirety of the retention period and this setting cannot be disabled once enabled.
...
* Allow access from Private endpoints
...
Deployment could take some time.

*When you see “Deployment succeeded” go to *resource group* 

* Copy the directory ID

When the key vault is created, copy the *directory ID*

* Objects > Keys > Generate / Import 

=== Ensure you have the correct permissions set up

So in this case, we went to 

It will highlight it in grey and then just press *next*

NOTE: Want to have the * Key Vault Crypto Officer * permission - that is really important!

* When the key is created, record the Key Identifier
...
In order for Neo4j to use the provided key, you have to grant Neo4j permissions to use the key.

== Azure Key Vault Checklist
Ensure your Azure Key Vault complies with the requirements below:

* The Key Vault is in the same region as the Neo4j instance you want to encrypt
* The Key Vault has the "Enable purge protection" option enabled.
* In the Access configuration settings of the Key Vault, the "Permission model" is set to "Azure role-based access control".
* In the Access configuration settings of your Key Vault, under "Resource access" the "Azure Disk Encryption for volume encryption" option is enabled
* In the Networking settings of your Key Vault, the "Enable public access" option is enabled

NOTE: You cannot delete an active key, that is being used by an instance.

NOTE: 
If you see a screen that says "Need admin approval" 
You may need to reach out to your internal IT team to ask for admin permission to install Azure app.
The customer owns their tenant - so Neo4j suport won't be able to do that for you. 
