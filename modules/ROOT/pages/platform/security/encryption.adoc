...
CAUTION: It's critical to differentiate between *Azure Key Vault* and Azure Key Vault Managed HSM to ensure proper selection for your key management needs. This document focuses on using *Azure Key Vault*.
...
. Permission model: Enable Azure role-based access control.
. Resource access: Enable Azure Disk Encryption for volume encryption.
...
. Select *Create*
. You should see “Deployment is in progress” (Deployment could take some time).
. When *Your deployment is complete* go to *resource*.
...
. Tap *Key Vault Administrator* - this will highlight the row in grey, and then press *next*.
. Members: select members (search by name, and when you have added the relevant person, press *select*).
. *Review + Assign*.
...
. Display client ID (Neo4j CMK Application ID) and application name in Aura Console.
. Create a Service Principal linked to the Neo4j CMK Application within the same Azure AD tenant where customer’s key is located by running.
. The Azure CLI command you need to run is `az ad sp create --id clientID`. For more information see the link:https://learn.microsoft.com/en-us/cli/azure/ad/sp?view=azure-cli-latest#az-ad-sp-create-examples[Microsoft documentation for Azure CLI]

==== Add role assignment to the key

. Go to *Access control (IAM).*
. *Add role assignment.*
. Search *Key Vault Crypto Officer*.
. Tap *Key Vault Crypto Officer* - this will highlight the row in grey, and then press *next*.
. Assign access to *User, group, or service principal*, type the Neo4j CMK Application ID, the app should appear.
. *Review + Assign*