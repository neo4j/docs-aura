...
`SecurityLogsReportMessage` is the message sent from the server to the Fleet Manager service containing security log data.
It is only sent if security logging is enabled explicitly through Aura Console.
...
|Unique identifier for the server
|`logs`
|Collection<SecurityLogsDefaultAggregation>
|Security logs data
|`logs[]`
|SecurityLogsDefaultAggregation
|A specific security log in the set
|`logs[].message`
|String
|Log event main messag
|`logs[].level`
|String
