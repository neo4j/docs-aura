...
| `is_system_db_writer`
| boolean
| Whether this server allows writing to system DB
|===
...
| `migration_step`
| String
| Name of the migration step to update status for
| `started_at`
| Instant
| Timestamp when the migration step started
| `completed_at`
| Instant
| Timestamp when the migration step completed
| `logs`
| MigrationStepLogs
| Logs from the migration step, if any
| `logs.stdout`
| String
| Standard output from the migration step
| `logs.stderr`
| String
| Standard error output from the migration step
| `error`
| MigrationStepError
| Error details from the migration step, if any
| `error.message`
| String
| Error message
| `error.stacktrace`
