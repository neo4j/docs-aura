...
// TODO author: keep these version values in sync with the product.
:min-version: 2026.05.0
:min-version-plugin: 5.26.0
:min-plugin-version: 1.2.0
:min-version-analysis: 2026.03.0

// TODO author: 1-2 sentence overview of the feature and when to use it.
...
* *Manual* -- you create and upload the database dump yourself using `neo4j-admin`, following an in-console checklist.

// TODO screenshot:: migrate-to-aura-entrypoint.png (the "Migrate to Aura" entry point on a deployment)

[NOTE]
====
This page covers the console-based migration available through Fleet Manager.
For the fully manual `neo4j-admin` dump-and-upload workflow (for example, for deployments that do not meet the requirements below), see xref:tutorials/migration.adoc[Migrate from self-managed Neo4j to Aura].
====
...
// TODO author: confirm wording; values are pulled from the product source.
* You must have the *Project admin* role (the `deployments:write` permission).
* Only one migration can run per deployment at a time.

// TODO author: note about automatic workload analysis below requiring Neo4j {min-version-analysis}+.

[NOTE]
====
// TODO author: confirm whether Community Edition has any additional limitations for migration.
====
...
// TODO screenshot:: migrate-to-aura-menu.png
...
// TODO screenshot:: migrate-to-aura-confirm-modal.png
...
Only some 4.4 driver versions work with Aura, so updating to 5.x drivers or later is recommended.
// TODO author: link to driver support article (https://support.neo4j.com/s/article/10845868658579).

*Identify queries or workloads that can't run on Aura*::
You can do this automatically or manually:
+
* *Automatic* -- analyzes collected query-log workload and lists unsupported queries (with the functions/procedures used, execution count, and when last recorded).
Requires Neo4j {min-version-analysis} or later and that *Migration workload collection* is enabled.
You can enable it for 24h from the wizard, or on the instance card under *Inspect > Settings*.
+
[NOTE]
====
Analysis only covers workload captured while collection was enabled, and may miss some function or procedure calls.
Treat the result as a guide, not a guarantee -- double-check your workload.
====
+
* *Manual* -- review your workload yourself. Aura does not allow custom plugins or arbitrary procedures and functions:
+
...
// TODO screenshot:: migration-wizard-compatibility.png
...
The wizard pre-fills a recommended size based on your deployment's usage and hides unsupported (too-small) sizes and the Free tier.
The estimated cost is shown before you create the instance.
// TODO author: note role/billing requirements surfaced here (org admin/owner and billing set up for paid instances).
...
* Not be an AuraDB Free instance (not supported).
// TODO author: link to Aura sizing/pricing guidance.

// TODO screenshot:: migration-wizard-destination.png
...
----
+
// TODO author: include Windows variants (bin\neo4j ...) and the verify/note-size/restart sub-steps if desired.
...
[IMPORTANT]
====
After a successful upload, the Aura instance is in a `LOADING` state and unavailable until the dump is processed. This can take some time depending on database size.
====
+
...
// TODO screenshot:: migration-wizard-transfer.png
...
// TODO author: mention the attempts selector (re-running a migration creates a new attempt).
// TODO screenshot:: migration-status.png

== Related pages

* xref:tutorials/migration.adoc[Migrate from self-managed Neo4j to Aura] -- manual `neo4j-admin` dump-and-upload workflow.
* xref:fleet-management/setup.adoc[Add a deployment] -- set up monitoring for a self-managed deployment.
* xref:fleet-management/overview.adoc[Overview] -- what Fleet Manager does.
