...
This option takes a similar approach as *Generate from schema*, but when no constraint information exists in the source, it attempts to infer it with GenAI by identifying likely primary and foreign key constraints.
This is better suited for data coming from relational sources. 
It is less suitable for data that is already in node/relationship structure, or for extracting graph structure from single files.
