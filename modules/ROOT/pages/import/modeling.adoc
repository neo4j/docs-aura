...
Once you have defined your data source, you can choose to create a model from the data, in which case you are taken to the *data model panel*.
At this point, a *model ID* is generated and appended to the URL.

For example, in the URL `console.neo4j.io/tools/import/models/0952dbcf-bc71-4413-8575-f8ffd4551234`, the model ID is `0952dbcf-bc71-4413-8575-f8ffd4551234`.
...
This option uses primary and foreign key constraints to equate tables to nodes and relationships, with naming for nodes and relationships simply derived from the table names.
...
*Generate with AI* takes a similar approach as *Generate from schema*, but when no constraint information exists in the source attempts to infer it with GenAI. GenAI tries to identify likely primary and foreign key constraints and then going through the same approach as the deterministic algorithm above. This is better suited to data coming from relational sources and there are currently some limitations on handling link tables. It doesn't work so well on data that is already in node/relationship structure, nor extracting graph structure from single files.
