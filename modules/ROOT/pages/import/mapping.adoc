...
By default, Import uses the property with `id` in its name as the ID.
If there is only one property, it is selected automatically.
If none of the columns in the mapped file meet this condition, or if more than one does, you have to manually select which property to use as the node ID.
