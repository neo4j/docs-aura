...
Aura instances support public access, private access, or both - and project admins can configure network access to an instance in the Aura console. 
...
== Query (Browser) and Explore (Bloom) access

When public traffic is disabled, Query (Browser) and Explore (Bloom) are not accessible via the public internet. 
...
Alternatively you can set up a VPN (Virtual Private Network) in your VPC and connect to Query (Browser) and Explore (Bloom) over the VPN.
...
Without private endpoints, you access Query (Browser) and Explore (Bloom) over the internet:

image::privatelink_01_before_enabling.png["Architecture overview before enabling private endpoints"]

When you have enabled private endpoints and disabled public internet access, to continue accessing Query (Browser) and Explore (Bloom), you can either connect via HTTPS (port 443), or use a private endpoint.

To access Query (Browser) and Explore (Bloom) over a VPN, you must ensure that:
...
image::privatelink_03_browser_bloom_over_vpn.png["Accessing Query (Browser) and Explore (Bloom) over a VPN"]
=====
...
When you have enabled private endpoints and disabled public internet access, you can no longer connect Query or Explore to your instances over the public internet:
