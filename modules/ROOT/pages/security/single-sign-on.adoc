...
Role mapping links a user’s identity from an identity provider (such as Okta or Microsoft Entra ID) to a specific instance role, based on attributes passed during authentication, like group or department.
...
The Entra group name can not be used but rather the UUID should be used.

For Azure, the role mapping should look something like:
`"eg680880-eg92-4bx0-9eg0-eg161cdfxa81"=admin;` where the UUID is the Object ID of the group in Microsoft Entra ID. Not the group name.
====
