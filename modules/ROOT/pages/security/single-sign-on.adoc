...
*Note:* An Organization with SSO enabled as a login method for the Aura Console can deactivate the login methods listed above.
...
* Authorization: Role management for projects, billing, and metrics is not currently handled via the IdP, and needs to be managed either via the UI or link:https://neo4j.com/docs/aura/platform/api/specification/?urls.primaryName=Aura%20v2beta1#/users[Aura API].
...
.. For the Discovery URI, append `/.well-known/openid-configuration` to your Okta domain. For example: `https://<your-okta-domain>/.well-known/openid-configuration`
