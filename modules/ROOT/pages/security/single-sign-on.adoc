...
Single Sign-On (SSO) enables organization owners and organization admins to use their organization’s identity provider (IdP) to authenticate users so they can access the Aura console and Aura instances.
...
[IMPORTANT]
====
Note that SSO does *not* apply retroactively.
If you set it up, only *new* instances are affected.
To apply SSO to existing instances, you need to clone the instances to new instances, and thus work around this limitation.
====
...
When used as login method for instances within Projects in this Org, it ensures that users logging in with SSO have access to the database instances within the project.
