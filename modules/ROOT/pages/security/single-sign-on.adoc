...
.. Select if you want the SSO config to be applied to organization logins, to specific projects within the org, or both
.. Enter a Display Name
.. For IdP Type select Okta
.. For Client ID enter the Client ID field from the Okta app details page
.. For Client Secret enter copy the client secret from the Okta app details page
...
.. so the final URL should look something like https://dev-123-admin.okta.com/.well-known/openid-configuration
.. Configure any additional settings as needed
