...
`aws-neo4j-sso` is a group name from the groups claim in the token and `editor` is the Aura role assigned to any user in that group.

== Instance-level SSO support

[NOTE]
====
Support is required to configure instance-level SSO.
====
...
Instance ID can be found at xref:managing-instances/instance-details.adoc[instance details].
