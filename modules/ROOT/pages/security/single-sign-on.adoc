...
There are two different ways to secure:
...
* *Use as login method for instances within Projects in this Org:* The SSO log in will be on the instances. Impacts new database instances created within that project.

[IMPORTANT]
====
Single sign-on for instances only works for AuraDB Business Critical and AuraDB Virtual Private Cloud. 
It does not cover AuraDB Professional instances.
====

[IMPORTANT]
====
Existing customers with existing instances that want to turn on single sign-on, need to know it doesn't affect existing instances. 
It will only affect new instances that you create. 
A workaround is to clone those instances. 
Once SSO is configured, clone those instances to a new instance, and the new instance will be protected by SSO.
====
...
.SSO configuration
[.shadow]
image::security/sso/sso-configuration.png[A screenshot of the SSO configuration,320,240]
...
*Use as login method for instances within Projects in this Org:* ensures users logging in with SSO have access to the database instances within the project.
