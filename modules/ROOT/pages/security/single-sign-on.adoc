...
`aws-neo4j-sso` is a group name from the groups claim in the token
`editor` is the Aura role assigned to any user in that group.
...
* Role mapping specific IdP groups to a database instance
* Adding custom claims (e.g., beyond `groups`)
* Updating SSO settings on already running instances

[NOTE]
====
Changes to a SSO configuration do not retroactively apply to existing AuraDB instances.
If an instance already exists and you enable SSO afterward, the new role mappings won’t be applied automatically — the order of operations matters.

However, if you need to update the SSO settings or role mappings on an existing instance, Neo4j Support can help make that happen.
====
...
Project ID can be found at link:https://neo4j.com/docs/aura/visual-tour/#_settings[project settings]

Instance ID can be found at https://neo4j.com/docs/aura/managing-instances/instance-details/[instance details]
