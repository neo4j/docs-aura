...
== Role mapping
 ...
 role mapping connects a user’s identity from your identity provider (like Okta or Microsoft Entra ID) to a specific role in Aura based on attributes from the SSO token, like their group membership.

 SSO is enabled for your Aura Org or Projects.
A user logs in using SSO — Aura receives a token with identity info.
Aura reads that token and uses role mapping to decide:
✨ “This person is in the ‘Data Editors’ group — they get the EDITOR role.”
✨ “This person is in the ‘Interns’ group — VIEWER only, please.”

🧩 This happens at the project/instance level, not at the broader organization level. (Org roles still need to be assigned manually in the Aura console.)
...
[NOTE]
====
Support is needed to configure instance-level SSO due to the current Aura infrastructure.
====

What support can do:

* Map IdP groups to instance roles
* Add custom SAML claims (e.g., specific to an instance)
* Update SSO settings on live instances
...
If you require support assistance, visit link:https://support.neo4j.com/[Customer Support] and raise a support ticket using this template:

==== Template message:

Project ID: [Insert]   https://neo4j.com/docs/aura/visual-tour/#_settings

Instance ID: [Insert] https://neo4j.com/docs/aura/managing-instances/instance-details/
