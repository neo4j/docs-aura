...
= Single Sign-On (SSO)
:description: This page describes how to set up SSO in Neo4j Aura.
...
SSO can be applied on the organization level, which means that it is the login method to the console itself, this is referred to as *Console SSO*.
Or, it can be applied on instances, either by project, or selected individually, known as *Instance SSO*.
...
* Enables users to log into the Aura Console via your Identity Provider (IdP) to manage the lifecycle of your instances.
* It controls how team members log in to manage billing, view metrics, create or delete projects, and provision new databases.
* Role management for projects, billing, and metrics is not currently handled via the IdP, and needs to be managed either via the UI or Aura API.
...
* Enables authentication for users or applications directly into specific Neo4j DBMS/instances (e.g., via Neo4j Browser, Bloom, or drivers) within an Aura Project.
* It ensures that when someone connects to a database instance within a specific project, they authenticate against your Identity Provider (IdP).
...
To simplify administration, a single SSO configuration can be used for both the Aura Console and database instances by selecting the corresponding checkboxes during setup: *"Use as login for the Organization"* and *"Use as login method for instances within Projects in this Organization"*.
These settings serve as the default for all instances within a project.
If you don't check any of the boxes, you can still add the configuration and apply it to organizations, projects and/or instances later.

It is recommended to group instances to projects based on their shared SSO login requirement.
...
Some legacy configurations may exist in an orphaned state.
It is recommended to create new configurations to utilize current SSO management capabilities where possible.
...
* Use as login method for instances within projects in this organization
...
== Linking SSO to projects and instances

Once you have added an SSO configuration, you can use the link icon to link the configuration to a project and or individual instances.
If you link a *project*, *all* new instances created within this project will require SSO login.
If you link an individual *instance*, the SSO configuration will be applied to that instance immediately.

[.shadow]
.Link a configuration to projects and/or instances
image::granular-sso.png[width=800]
...
If you have overrides on instances, the overridden fields remain frozen but everything else updates with an SSO configuration update.

For example, if role mapping is overridden on an instance and the linked SSO configuration username claim is updated, the instance still receives this update when the configuration is applied.
If the linked SSO configuration's role mapping is edited, the instance override prevails.
