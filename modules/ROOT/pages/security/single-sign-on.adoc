...
The SSO option for `group` to `role` mapping can map any role defined in the DBMS instance.

Role mapping maps a group in the customer's SSO provider to an instance role.
If the same SSO configuration for multiple instances is used, that same mapping applies.

When SSO is enabled at the project level, Aura uses role mapping to determine what access a user should have to instances based on their IdP group membership.

Multiple group to role mappings can be defined, and multiple roles can be given to any SSO group.
The role field can be a comma separated list.

This access then applies to *all newly created* instances the user has access to.

* *Azure:* the role mapping should look something like:
`"aj680880-ag92-4bx0-9eg0-eg161cdfxa81"=admin;` where the UUID is the Object ID of the group in Microsoft Entra ID, not the group name.

* *Microsoft Entra ID:* the Entra group name can not be used but rather the UUID should be used.

* *OIDC:* For example, `aws-neo4j-sso` is a group name from the groups claim in the token and `editor` is the instance role assigned to any user in that group.

[.shadow]
.Example role mapping in the Aura console
image::security/role-mapping-ui.png[width=500]
