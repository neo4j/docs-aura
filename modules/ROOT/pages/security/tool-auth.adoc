...
:page-role: aura-db-free aura-db-professional aura-db-business-critical aura-ds-professional aura-graph-analytics
...
When enabled, users connect to an instance via Query or Bloom with a predefined database role matching their console role (see xref:user-management.adoc#roles[User management - Roles] for more information about roles and privileges.)

If this setting is disabled, all users are required to connect to graph tools with a database username and password.

[NOTE]
====
Tool authentication with Aura user is enabled by default on new organizations created after May 29th 2025.

Note that tool authentication with Aura user is not available for Virtual Dedicated Cloud (VDC).
====

This feature can be enabled and configured from *Security* on the organization level, available by selecting the organization name in the dropdown menu.

[.shadow]
image::org-security.jpg[]
