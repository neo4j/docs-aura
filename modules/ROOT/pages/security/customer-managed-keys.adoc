...
When using a Customer Managed Key, all data at rest is encrypted with the key, including backups.
...
For more information, see xref:managing-instances/backup-restore-export.adoc#neo4j-admin-upload[Upload an existing Neo4j database].

This is because `neo4j-admin push-to-cloud` does *not* work at all with the new console and you need `neo4j-admin database upload` and version  +5.26.7 or +2025.04 
====
...
. In the *Role* tab, select *Key Vault Crypto Service Encryption User*.
. In the *Member* tab, select *User, group, or service principal*.
...
[IMPORTANT]
====
Wait at least 24 hours after rotating a key before disabling the old version to allow the change to take effect in Azure.
====
