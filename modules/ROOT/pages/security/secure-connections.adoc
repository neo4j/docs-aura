...
[role=label--aura-db-dedicated label--aura-ds-enterprise]
== VPC isolation
...
To continue accessing these tools, xref:getting-started/connect-instance.adoc#_https[connect via HTTPS (port 443)], this is helpful when network security blocks Bolt (port 7687), e.g. when a private link is set up on the database with public traffic disabled.
Alternatively you can set up a VPN (Virtual Private Network) in your VPC and connect to Query and Bloom over the VPN.
...
Without private endpoints, you access the tools Query and Bloom over the internet:
...
When you have enabled private endpoints and disabled public internet access, you can no longer connect Query and Bloom to your instances over the internet.
To continue accessing the tools, you can set up a VPN.

To access Query and Bloom over a VPN, you must ensure that:
...
# if you are using Graph Analytics (Aura Graph Analytics or AuraDS)
