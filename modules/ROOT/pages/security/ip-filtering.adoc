...
* AuraDB Virtual Dedicated Cloud and AuraDS Enterprise support up to 100 allowed IP ranges.
...
[NOTE]
====
The *Aura Agent* organization setting needs to be *disabled* in order to create an IP Filter.
====
...
To use `neo4j-admin database upload` with IP filtering enabled, you must allow control plane access.

In the IP filter configuration, enable the *Control Plane Access* option.
This allows the temporary connection required to upload data without needing to manually add control plane IP addresses to the allow list.

[NOTE]
====
Enabling *Control Plane Access* replaces the need to manually maintain control plane IP addresses in your IP allow list.
====
