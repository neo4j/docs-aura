...
. In the Aura console, go to *Organization Security > IP Filtering*
...
* Users only need to add their local IP address to the Allow List to access Aura from tools such as Query, Bloom, and Neo4j Desktop.

== Using `neo4j-admin database upload` with IP filtering

When setting up an IP filter, *Enable Control Plane access* to allow network access for the Aura Control Plane to selected entities, required when using `neo4j-admin database upload`.
