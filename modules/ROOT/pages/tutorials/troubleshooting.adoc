...
If you connect to an AuraDB instance using a driver, you can encounter the following errors:
...
This can be due to an erroneous URI, a network/firewall issue, or that **the instance is paused**.
AuraDB Free instances automatically pause after 3 days of inactivity, and a paused instance's hostname will not resolve.

Before checking your URI, credentials, or network configuration, first confirm the instance status at https://console.neo4j.io. 
If the instance shows as *Paused*, *Resume* the instance and wait a few minutes for it to become available again before retrying your connection.