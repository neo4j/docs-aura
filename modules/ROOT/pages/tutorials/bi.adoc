...
If the folder is not immediately visible, select `Go -> Go to Folder` in the Finder application to open the folder manually.
...
If the connection fails with a `Generic JDBC connection error`, make sure you installed the Neo4j driver in the correct location.
