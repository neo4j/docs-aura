...
With the new console the only compatible versions of `database upload` that can work are 5.26.7 and later, 2025.04 and later.
...
Use the link:https://neo4j.com/docs/operations-manual/4.4/tools/neo4j-admin/push-to-cloud/[`neo4j-admin upload`] command.

[source,shell,subs=attributes+]
----
bin/neo4j-admin upload --dump={dump-folder}/file.dump --bolt-uri={aura-uri} --overwrite
----
=====
