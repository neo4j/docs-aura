...
* The Migration Readiness Report in the Aura console helps Neo4j Aura 4 users to get code ready for migration.
====
...
B-tree indexes have been replaced by RANGE, POINT, and TEXT indexes. 
These indexes are not automatically created during migration, so you must manually create an equivalent index for each b-tree index in Neo4j 4.
...
Read more in the link:{neo4j-docs-base-uri}cypher-manual/4.4/indexes-for-search-performance/#indexes-future-indexes[Cypher Manual] / Read more in the Cypher 4.4 docs > Future Indexes.
