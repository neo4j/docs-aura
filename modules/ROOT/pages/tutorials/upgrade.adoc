...
When migrating to a new Aura instance, users and passwords, custom roles and privileges are not copied as part of the cloning or dump/restore process.
To ensure access remains consistent after migration, recreate users and roles in the target instance.

=== Prerequisites

* Sign into your source database because you will run the Cypher queries on it.
...
This script will generate Cypher for roles that need to be created if they do not exist already.
...
The user will be able to access the resource if their roles have a `GRANT` (allowlist) and do not have a `DENY` (denylist) relevant to that resource.
...
When we created users in the first step, by default all users got the `PUBLIC` role which provides access to the default database and allows the execution of all procedures and user-defined functions. 
Therefore the script below will exclude the `PUBLIC` role and roles with no users mapped.
...
This will generate Cypher scripts to map users to roles. 
