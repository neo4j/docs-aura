...
TIP: Follow along with a notebook in https://colab.research.google.com/github/neo4j/docs-aura/blob/main/notebooks/Connecting_with_Python_(GDS_client).ipynb[image:colab.svg[Colab,24] Google Colab^]

This tutorial shows how to interact with AuraDS using the link:{neo4j-docs-base-uri}/graph-data-science/current/python-client/[Graph Data Science (GDS) client^] or the link:{neo4j-docs-base-uri}/driver-manual/current/get-started[Python Driver^]. In the following sections you can switch between client and driver code clicking on the appropriate tab.
