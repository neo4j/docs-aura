...
For more information about the https://neo4j.com/docs/graph-data-science/current/model-catalog/model-catalog/[Model Catalog], head over to the Neo4j Graph Data Science manual

include::partial$aurads/setup.adoc[]

== Example graph

We're going to create a graph within the instance to use in-memory and create a model from.
...
== Loading example data into memory
...
If we list the model catalog again, we'll see that the stored flag has been set to `true`
...
== Sharing the Model with other instance users
...
gds.run_cypher(create_a_new_user_query)
----
=====
...
How to drop the test user, in-memory graphs & models, but also how to delete the model from disk.
...
gds.run_cypher(drop_example_user_query)  
----
=====
