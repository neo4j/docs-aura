...
Native projections are used to load into memory a graph stored on disk. The https://neo4j.com/docs/graph-data-science/current/graph-project/[`gds.graph.project`^] procedure allows to project a graph by selecting the node labels, relationship types and properties to be projected.
