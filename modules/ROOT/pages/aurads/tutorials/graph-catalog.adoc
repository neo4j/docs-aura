[[graph-catalog]]
= Using the Graph Catalog and projecting graphs
:description: This page describes how to use the graph catalog and in-memory graphs.
...
== List the projected graphs

The Graph Catalog can be used to retrieve information on the projected graphs. For example, the https://neo4j.com/docs/graph-data-science/current/graph-list/[`gds.graph.list`^] procedure can be used to list all the graphs currently stored in memory:
...
[source, python, subs=attributes+]
----
# Drop the graph object directly (no graph name needed)
...
with driver.session() as session:
  # Run the Cypher procedure
  result = session.run(drop_graph_call).data()
  # Prettify the result
  print(json.dumps(result, indent=2, sort_keys=True, default=default))
----
=====
