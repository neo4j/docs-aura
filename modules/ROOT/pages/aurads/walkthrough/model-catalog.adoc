...
[source, python, subs=attributes+]
----
g_graphsage, result = gds.beta.graphSage.train(
  g,
...
[source, python, subs=attributes+]
----
result = gds.alpha.model.store(g_graphsage)
...
g_graphsage_public = gds.alpha.model.publish(g_graphsage)
...
gds.graph.drop(g_graphsage)

# Delete the model from disk
gds.beta.model.drop(g_graphsage_public);

# Delete the model from disk
gds.alpha.model.delete(g_graphsage_public)
