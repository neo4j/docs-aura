...
This page describes the various ways you can get data into a Neo4j AuraDS database.

[NOTE]
====
*Note:* The process of importing or loading data requires you to xref:aurads/getting-started/create-database.adoc[create an AuraDS database] beforehand.
====
...
There are two ways you can import data from an existing Neo4j database into an AuraDS database.
...
`Push-to-cloud` is a Neo4j Admin command that you can run to upload the contents of a Neo4j database into an AuraDS database, regardless of the database's size.
...
You can load data from a *_.csv_* file into an AuraDS database with the https://neo4j.com/docs/cypher-manual/current/clauses/load-csv/[`LOAD CSV`] Cypher statement within Neo4j Browser.
For instructions on how to open an AuraDS database with Neo4j Browser, see xref:aurads/getting-started/access-database.adoc[].

There are some limitations to consider when loading a *_.csv_* file into an AuraDS database:
