...
There are a few different methods of interacting with an AuraDS instance after its creation:

* <<_exploring>> - Explore the graph by visually interacting with graph data.
* <<_managing>> - Manage local and cloud databases.
* <<_querying>> - Query (and optionally viewing) graph data using the Cypher query language.

[TIP]
====
*Tip:* For first-time users, we recommend using the <<_neo4j_browser>>.
====
...
You can query data in an AuraDS instance using Cypher. Cypher is the declarative graph query language created by Neo4j and can be used to query, update, and administer your AuraDS instance.

You can run Cypher statements through the <<_neo4j_browser>> or the <<_neo4j_cypher_shell>>.
