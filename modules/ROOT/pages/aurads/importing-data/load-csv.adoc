...
A *_.csv_* file can be loaded into an AuraDS instance using the https://neo4j.com/docs/cypher-manual/current/clauses/load-csv/[`LOAD CSV`] Cypher clause. For security reasons it is not possible to load local *_.csv_* files, which must be instead publicly accessible on HTTP or HTTPS servers such as GitHub, Google Drive, and Dropbox.

In this tutorial we will load some CSV files from https://data.neo4j.com:
...
We are now ready to load the CSV files from their URIs and create nodes and relationships from the data they contain. In the following examples:

* `MERGE` is used with the indexed labels to take advantage of the constraint created in the <<_create_constraints>> section.
* `ON CREATE SET` is used to set the value of a node/relationship property when a new one is created.
...
* `FIELDTERMINATOR` is used to explicitly set the terminator character (although the default is already `,`).
* `MATCH` is used to find nodes to create a relationship between.

Note that the CSV files in this tutorial are curated, so some assumption are made for simplicity. In a real-world scenario, for example, a CSV file could contain multiple rows that would assign different property values to the same node; in this case, an `ON MATCH SET` clause must be added to ensure this case is dealt with appropriately.
