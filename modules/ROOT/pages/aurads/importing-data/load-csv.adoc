...
WARNING: For best performance when using `MERGE` or `MATCH` with `LOAD CSV`, make sure an index or a unique constraint has been created on the property used for merging. For more information, follow the link:{neo4j-docs-base-uri}/getting-started/current/cypher-intro/load-csv/#_prepare_the_database[Import data] tutorial in the Cypher documentation.
...
In the following examples, `LOAD CSV` is used with the `WITH HEADERS` option to access the fields in each `row` by their corresponding column name.
