...
You can get a view of instance resources from *Metrics* on the instance card.
Note that metrics are *not* available for AuraDB Free.

You can also monitor metrics of an instance from *Operations -> Metrics* in the left-hand navigation of the console.
Then you can navigate between tabs for resources, instance and database metrics.
...
The *[...]* menu on each card allows you to see more information about the metric in question.
...
For multi-database instances, from the _Database_ tab you can switch between databases using the dropdown.

[.shadow]
.MultiDB metrics dropdown
image::/multidb/multidb-metrics.png[]