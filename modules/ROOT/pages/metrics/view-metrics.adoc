...
* From the instance card, expand the *Metrics* section to view a summary, or select *View all metrics* to open the full metrics page.
* From the sidebar, go to *Operations* → *Metrics*m for the full metrics page.

When accessed from an instance, metrics are shown for that instance by default.
In the full metrics page, you can switch between *Resources, Instance,* and *Database* metrics.
...
Select the info icon to see information about a particular metric, and you can expand the graph.
...
[role=label--aura-db-business-critical label--aura-db-dedicated]
== Detail view
...
If your instance contains multiple databases, select the database from the dropdown in the Metrics view.
Metrics are shown for the selected database.
