...
* Transform data sources accessible via OneLake Shortcuts including but not limited to in AWS S3, Azure Data Lake Storage, Google Cloud Storage.
...
If your Fabric capacity is outside of one of these regions, then you will need to select your preferred region from this list.
...
If your Fabric capacity is outside of one of the supported Aura regions, then you will need to select your preferred region from the provided list.
...
During or at the end of your 14-day trial, you can upgrade to a paid plan using one of the following options:
...
. Navigate to the provided billing URL and login to Aura Console using `Continue with Microsoft` option with the same account you use to login to Fabric.
. Add your payment method on the billing page and this will transition your account to a paid account.

=== Azure Marketplace

. Open link:https://azuremarketplace.microsoft.com/en-us/marketplace/apps/neo4j.neo4j_aura_professional[Neo4j AuraDB Professional (pay-as-you-go)] product page on Azure Marketplace.
. Follow instructions to purchase the plan by selecting `Get It Now`.
. At the end of the process, you will have a new Aura project that is linked to Azure Marketplace.
When creating new graph dataset instances, you will be presented with a list of projects that you can create AuraDB instances where you can select this new project and use the workload without any trial limitations.

[NOTE]
====
Please note that the upgrade via the Azure Marketplace will result in a new Organisation and Project being created in Aura, which means that you will need to recreate your graph dataset item from scratch.
====