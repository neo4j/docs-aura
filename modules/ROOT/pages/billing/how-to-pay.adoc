...
Get started by creating an Aura account, no sales team is needed.
Cancel at any time, with no extra charge.
Pay as you go has no expiration date, it Continues as long as you use Aura.

Usage is billed monthly for the previous month. At every billing cycle, usage from the previous month is priced, invoiced and then charged to the customer’s payment method.
...
There's two separate channels for purchasing ACUs:
...
. If the customer runs out of ACUs before their expiration date, they are be invoiced for the overage.
