...
* Usage billed monthly, for the previous month. At every billing cycle, usage from the previous month is priced, invoiced and then charged to the customer’s payment method.
...
* Buy Aura Consumption Units (ACUs)

* Does not renew automatically.
* Signing up requires the sales team.

* There's two separate procurement channels for purchasing ACUs:
...
The contract is executed as a private offer. 
Neo4j instructs the marketplace to invoice the customer for a number of ACUs and nothing else. 
...
* The nominal value of an ACU is $1 (excluding any discounts at the point of credits purchase). If a customer runs out of ACUs and needs to pay the overage in dollars, the exchange rate of ACU usage to dollars is 1 ACU = $1.
