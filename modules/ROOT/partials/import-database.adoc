...
Use the *import database* process using the Aura console UI to import a _.backup_, _.dump_ or _.tar_ file as long as it’s less than 4GB. 

Use the <<_neo4j_admin_database_upload>> command if the size of the  _.backup_, _.dump_ or _.tar_ file exported from a database is greater than 4GB.

It is recommended that the data in the file is exported from a Neo4j database running version 4.4 or higher.
