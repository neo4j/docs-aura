...
Promise.all([runAPOCHelpQuery(5, apocHelpQuery), runAPOCHelpQuery(25, apocHelpQuery)])
    .then(res => {
        ...
        // Merge by name and accumulate versions
        const uniqueByName = merged.reduce((acc, proc) => {
            if (!acc[proc.name]) {
                acc[proc.name] = { ...proc };
            } else {
                acc[proc.name].cypherVersions = Array.from(new Set([...acc[proc.name].cypherVersions, ...proc.cypherVersions]));
                acc[proc.name].isDeprecatedCypher5 = acc[proc.name].isDeprecatedCypher5 || proc.isDeprecatedCypher5
                acc[proc.name].isDeprecatedCypher25 = acc[proc.name].isDeprecatedCypher25 || proc.isDeprecatedCypher25
            }
            return acc;
        }, {});
